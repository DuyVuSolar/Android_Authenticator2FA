...
class CircularProgressView @JvmOverloads constructor(
    ...
    private fun setupAttr(context: Context, attrs: AttributeSet?) {
        ...
        if (attrs != null) {
            ...
            try {
                ...
                progressThumbScaleType = ProgressThumbScaleType.values().first{it.value == typedArray.getInteger(R.styleable.CircularProgressView_progressThumbScaleType, 1)}
                ...
                progress = typedArray.getFloat(R.styleable.CircularProgressView_progressCircle, 0f)
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (isShadowEnabled) {
            ...
            if (!multipleArcsEnabled && isProgressThumbEnabled) {
                ...
                when (progressThumbScaleType) {
                    ProgressThumbScaleType.POINT, ProgressThumbScaleType.RATE -> shadowThumbPaint.style = Paint.Style.FILL
        ...
        for (i in valuesToDrawList.indices) {
            ...
            if (!multipleArcsEnabled && isProgressThumbEnabled) {
                ...
                when (progressThumbScaleType) {
                    ProgressThumbScaleType.POINT, ProgressThumbScaleType.RATE -> thumbPaint.style = Paint.Style.FILL
    ...
    companion object {
        ...
        private const val DEFAULT_ANIMATION_MILLIS = 200
...
enum class ProgressThumbScaleType(val value : Int) { AUTO(1), POINT(2), RATE(3) }
