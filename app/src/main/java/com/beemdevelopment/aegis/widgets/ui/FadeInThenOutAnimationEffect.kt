package com.beemdevelopment.aegis.widgets.ui
...
internal class FadeInThenOutAnimationEffect(private val view: View) {
    ...
    fun go(playing: Boolean) {//playing : hide button, pause : Show button
        reset()

        val fadeIn = PropertyValuesHolder.ofFloat(View.ALPHA, 0f, 1f)
        val fadeOut = PropertyValuesHolder.ofFloat(View.ALPHA, 1f, if(playing) 2f else 0f)
        val expandX = PropertyValuesHolder.ofFloat(View.SCALE_X, 1f, 2f)
        val expandY = PropertyValuesHolder.ofFloat(View.SCALE_Y, 1f, 2f)
        val shrinkX = PropertyValuesHolder.ofFloat(View.SCALE_X, 2f, if(playing) 2f else 0f)
        val shrinkY = PropertyValuesHolder.ofFloat(View.SCALE_Y, 2f, if(playing) 2f else 0f)
