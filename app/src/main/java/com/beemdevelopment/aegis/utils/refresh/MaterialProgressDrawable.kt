...
class MaterialProgressDrawable(val context: Context, private val mAnimExcutor: View) : Drawable(), Animatable {
    ...
    private fun applyFinishTranslation(interpolatedTime: Float, ring: Ring) {
        ...
        val startTrim: Float = (ring.startingStartTrim + (ring.startingEndTrim - ring.startingStartTrim) * interpolatedTime)
        ring.startTrim =startTrim
        val rotation: Float = (ring.startingRotation + ((targetRotation - ring.startingRotation) * interpolatedTime))
    ...
    private fun setupAnimators() {
        ...
        val animation: Animation = object : Animation() {
            public override fun applyTransformation(interpolatedTime: Float, t: Transformation) {
                if (mFinishing) {
                    ...
                    val minProgressArc = Math.toRadians(ring.strokeWidth / (2 * Math.PI * ring.centerRadius)).toFloat()
                    val startingEndTrim: Float = ring.startingEndTrim
                    val startingTrim: Float = ring.startingStartTrim
        ...
        animation.setAnimationListener(object : Animation.AnimationListener {
            ...
            override fun onAnimationRepeat(animation: Animation) {
                ring.storeOriginals()
                ring.goToNextColor()
                ring.startingStartTrim = (ring.endTrim)
                if (mFinishing) {
    ...
    private class Ring(private val context: Context, private val mCallback: Callback) {
        ...
        fun drawTriangle(c: Canvas, startAngle: Float, sweepAngle: Float, bounds: Rect) {
            if (mShowArrow) {
                if (mArrow == null) {
                    mArrow = Path()
        ...
        fun setInsets(width: Int, height: Int) {
            val minEdge = min(width.toDouble(), height.toDouble())
            val insets = if (centerRadius <= 0 || minEdge < 0) {
                ceil((mStrokeWidth / 2.0f).toDouble())
            } else {
                (minEdge / 2.0f - centerRadius).toFloat()
            }
            this.insets = insets.toFloat()
        }
        ...
        fun invalidateSelf() {
            context.getDrawable(R.drawable.bg_transparent)?.let { mCallback.invalidateDrawable(it) }
        }
    }
    ...
    private class EndCurveInterpolator : AccelerateDecelerateInterpolator() {
        override fun getInterpolation(input: Float): Float {
            return super.getInterpolation(max(0.0, ((input - 0.5f) * 2.0f).toDouble()).toFloat())
        }
    }
    ...
    private class StartCurveInterpolator : AccelerateDecelerateInterpolator() {
        override fun getInterpolation(input: Float): Float {
            return super.getInterpolation(min(1.0, (input * 2.0f).toDouble()).toFloat())
        }
    }

    companion object {
        // Maps to ProgressBar.Large style
        const val LARGE: Int = 0
        ...
        const val STROKE_WIDTH_LARGE: Float = 3f
         ...
         */
        const val ARROW_WIDTH_LARGE: Int = 12
        const val ARROW_HEIGHT_LARGE: Int = 6
