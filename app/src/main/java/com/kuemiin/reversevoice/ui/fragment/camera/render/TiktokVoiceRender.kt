 ...
 * @date 2018-05-25
 */
class TiktokVoiceRender(context: Context, val widthRender : Int, val typeFilter : String) : QuestionRender(context) {

    var heightRender = 0

    init {
        heightRender = (widthRender * 1.4f).toInt()
        val sticker = Sticker()
        sticker.components = ArrayList()
        val component = Component()
        component.duration = 400
        component.src = "${CanvasFileFilter.getFilterFormatFile("typeFilter")}/${CanvasFileFilter.folderVoice}"
