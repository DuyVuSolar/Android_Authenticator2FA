...
public class Camera2Input extends FBORender implements SurfaceTexture.OnFrameAvailableListener, ISupportTakePhoto {
    ...
    public Camera2Input(IGLEnvironment render, CameraDevice camera, Size previewSize, CameraBuilder.SurfaceCallback callback) {
        super();
        this.mRender = render;
        this.mCamera = camera;
    ...
    protected void initGLContext() {
        ...
        // Fixed the problem of unclear preview screen
        ...
        try {
            mCallback.callback(mSurfaceTexture);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
